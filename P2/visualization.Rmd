...
The dataset would be downloaded from [1]. There are 95 csv files. Each of them contains contains 38 pieces of information (we called this _variables_) among different counties, including national record, in Liberia. [2]

Those 38 pieces of information are:
...
### 1: Aggregation + Data Munging 

Suppose all the csv files are already downloaded in the machine. Then, it is necessary to aggregate the files into a single workable dataset.
...
First, let's check if there are really 38 different variables.

```{r}
variableNames = unique(allData$variable)
```

But, there are `r length(variableNames)` difference variable names in all csv files. They are:

```{r echo=FALSE}
sort(variableNames)
```

It is necessary to rename a number of variables names. For example, remove "\n" from the variable names.
Furthermore, to ease for maniplulation, I converted the values of the variables into lowercase.
...
We finished the update about the variable names, now let's see the column names.

```{r}
sort(colnames(allData))
```
There are 19 columns among all csv files. There is a weird column namded "x".  I'd like to check how many row(s) with non-NA values in column x.

```{r}
allData[!is.na(allData$x), ] 
```

There are only 3 rows with non-NA values in column 'x'. It seems that those values in column x is not really meaningful (with value 0 or 1). I decided to drop this.

```{r}
allData$x = NULL #drop column x
```

Next, I would like to check the date format and see if they are consistent.

```{r}
unique(allData$date)
```
There are 94 Ebloa records between 6 June 2014 and 2 December 2014. From this information, we can learn that the Ebloa data was not updated every day in such close-to-half-year period. For the date format, as we can see that they are inconsistent. Some of them use the short form of the year (e.g. 14 for 2014) and some of them with the leading zero of the month (e.g. 09 for September). It is better to convert the date in consist way.
...
Refer to the diagram, the total number of deaths was increased from July to mid-October 2014. Montserrado County in Liberia with the most number of deaths in confirmed cases. Since the data did not record the total number of death in confirmed cases after mid-October, we have no idea if there is any decrease after mid-October.
...
Refer to the graph, we can see that the number of new probable cases is quite fluctuate from July to December 2014. There is a great increase in Lofa County in August. Furthmore, there are two peaks in Montserrado County in mid-September and mid-October. However, we can see that the number of probable case is decreasing after mid-October.

#### Total Confirmed Ebola Cases in Libera

When I first plotted the graph with the orginal data, I found that there is inconsistent value in the total confirmed Ebola cases in the data. Thus, I defined a function to remove the inconsistent value. 
...
Montserrado County had a great increased in the total number of confirmed Ebola cases at the beginning of September and in the mid-October. Lofa County also has a shape increase at the beginning of September. However, in Lofa County, the number of confirmed Ebola cases was only steadily increasing from October. 

### Choropleth map - The Number of New Ebola Cases (confirmed, probable and suspected) in Liberia

To get some idea how Ebola spread in Liberia, it would be great if we can shows the choropleth map about the number of new cases (confirmed, probable and suspected) in Liberia. I learnt how to plot choropleth from [3] and I downloaded the adminstrative boundary level 1 of Liberia from [4].

Since the orignal data about the new Ebola cases are seperarte into three variables: confirmed, probable and suspected, I need to calculate the total value of news cases for each county in each month. Then, I plot the 
corrsponding choropleth map accordingly.
...
Refer to these graphs, we can see that the protential source of the Ebola cases were from Lofa and Monsterrado in Liberia. Since Bong is located in between Lofa and Montserrado, Ebola spreaded to Bong in July. In addition, the counties next to Monserrado, i.e. Bomi and Grandbassa.

In Aug 2014, Lofa and Monserrado had a great increase in the number of new Ebloa cases and Ebloa spread outwards to counties such as Numba.

In September 2014, Monserrado had the most number of new Ebloa cases. There is also a great increases in Lofa, Bong, Nuba. As Monserrado would be the protential source of Eloba, the counties next to it, such as Margibi, Bomi and Grandassa, also had an increase in the new Ebloa cases. 

In October 2014, Monserrado reached around 750 new cases and there is a trend that Eloba spread to the boundary of Liberia.

In November and Decmeber 2014, the number of new cases were greatly decreased. It showed that the situation was under control. Although Monserrado still has around 400 cases, the other nearby counties did not have any more increase in the number of new cases.

### Conclusion

The potential sources location of Eloba in Liberia are Lofa and Monserrado. It spreads to the nearby counties in short period of time and greatly affected the other countries in September 2014. It seems that the spread was stopped, but, Montserrado still had a great number of new cases October 2014 which would  tigger another wave of spread. Lucky that the spread was stopped in November 2014. Since the data was in-complete in December (we only have data till 02 December 2014), we cannot draw the conclusion that if Ebola already stopped spreading to the other counties. From the news, the spread of Ebola should now be under-controlled.
